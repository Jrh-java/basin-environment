...
const week = ref<string>(dayjs().format("YYYY-MM-DD"))
...
function getWeekRange(dateStr: string) {
  ...
  const begin = d.subtract((day + 6) % 7, "day") // 周一为起始
...
async function fetchList() {
  loading.value = true
  try {
    const { begin, end } = getWeekRange(week.value)
    ...
    <template #searchSlot>
      <div class="search-bar" v-loading="loading">
        <div class="left">
          ...
          <el-date-picker v-model="week" type="week" format="YYYY 第ww周" value-format="YYYY-MM-DD" placeholder="选择周" />
