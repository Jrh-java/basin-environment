...
<template>
  <a-form class="smart-query-form">
    <a-row class="smart-query-form-row">
      ...
      <a-form-item label="类型" class="smart-query-form-item">
        <SmartEnumSelect style="width: 150px" v-model:value="queryForm.messageType" placeholder="消息类型" enum-name="MESSAGE_TYPE_ENUM" />
      </a-form-item>
  ...
  <a-table :data-source="tableData" :columns="columns" row-key="messageId" :pagination="false" bordered>
    <template #bodyCell="{ text, record, column }">
      ...
      <template v-if="column.dataIndex === 'title'">
        <span v-show="record.readFlag">
          <a @click="toDetail(record)" style="color: #8c8c8c">【{{ $smartEnumPlugin.getDescByValue('MESSAGE_TYPE_ENUM', record.messageType) }}】{{ text }}</a>
        </span>
        <span v-show="!record.readFlag">
