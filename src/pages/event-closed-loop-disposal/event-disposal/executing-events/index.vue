...
defineOptions({ name: "ExecutingEvents" })
...
// 督办
function handleSupervise(row: any) {
  ElMessage.success(`督办事件：${row.eventName}`)
}

// 反馈
function handleFeedback(row: any) {
  ElMessage.success(`提交反馈：${row.eventName}`)
}
</script>

<template>
  <div class="executing-events-container">
    <SearchTableLayout>
      <template #searchSlot>
        ...
        <el-card shadow="never" class="table-card">
          <el-table v-loading="loading" :data="tableData" style="width: 100%" border>
            ...
            <el-table-column label="操作" width="150" align="center" fixed="right">
              <template #default="{ row }">
                <el-button link type="danger" @click="handleSupervise(row)">
                  督办
                </el-button>
